...
class FitFrame(object):
    def __init__(self, 
        ...
        print('v1.6, 250131: (1) Update ELineModels line setup')
    ...
    def load_models(self, model_config):
        ...
        if np.isin(mod, [*model_config]):
            if model_config[mod]['enable']:             
                self.full_model_type += mod + '+'
                self.model_dict[mod] = {'cf': ConfigFrame(model_config[mod]['config'])}
                self.model_dict[mod]['specmod'] = ELineModels(w_min=self.spec_wmin, w_max=self.spec_wmax, use_pyneb=model_config[mod]['use_pyneb'],
                                                              cframe=self.model_dict[mod]['cf'], v0_redshift=self.v0_redshift, spec_R_inst=self.spec_R_inst) 
                self.model_dict[mod]['specfunc'] = self.model_dict[mod]['specmod'].models_unitnorm_obsframe
                if self.have_phot:
                    self.model_dict[mod]['sedmod'] = ELineModels(w_min=self.sed_wmin, w_max=self.sed_wmax, use_pyneb=model_config[mod]['use_pyneb'],
                                                                 cframe=self.model_dict[mod]['cf'], v0_redshift=self.v0_redshift, spec_R_inst=self.spec_R_inst, 
                                                                 verbose=False)
        mod = 'agn'
        if np.isin(mod, [*model_config]):
    ...
    def output_el(self):
        ...
        list_linked = np.where(el_mod.lineratio_n > 0)[0]
        for i_line in list_linked:
            i_main = 1 + el_cf.num_pars + np.where(el_mod.linename_n == el_mod.linelink_n[i_line])[0]
            for i_loop in range(n_loops):
                for i_comp in range(el_cf.num_comps):
                    el_mod.update_lineratio(self.output_el_lcp[i_loop, i_comp, 1+2], 
                                            self.output_el_lcp[i_loop, i_comp, 1+3], 
                                            self.output_el_lcp[i_loop, i_comp, 1+4])
                    self.output_el_lcp[i_loop, i_comp, 1 + el_cf.num_pars + i_line] = self.output_el_lcp[i_loop, i_comp, i_main] * el_mod.lineratio_n[i_line]
    ...
    def output_el_to_screen(self):
        ...
        for i_line in range(el_mod.num_lines): 
            names.append('{}'.format(el_mod.linename_n[i_line]))
...
class ELineModels(object):
    def __init__(self, w_min=None, w_max=None, cframe=None, v0_redshift=0, spec_R_inst=1e8, use_pyneb=False, verbose=True):
        self.w_min = w_min
        self.w_max = w_max
        self.cframe = cframe
        self.v0_redshift = v0_redshift
        ...
        if name in HI_lv_low_dict:
            ...
            if np.isin(element+str(notation), ['H1', 'He2']):
                ...
                if ~np.isin(element+str(notation), pyneb.atomicData.getAllAtoms(coll=True, rec=False)):
                    raise ValueError((f"{name} not provided in pyneb, please add it into ELineModels.initialize_linelist() manually."))
    ...
    def update_freemask(self):
        ...
        for i_comp in range(self.num_comps):
            mask_valid_n  = self.linerest_n > (self.w_min-50)
            mask_valid_n &= self.linerest_n < (self.w_max+50)
    ...
    def models_single_comp(self, wavelength, pars, list_valid, list_free):
        # update lineratio_n
        ...
        for i_free in list_free:
            model_sline = self.single_gaussian(wavelength, self.linerest_n[i_free], voff, fwhm, 
            ...
            for i_linked in list_linked:
                model_sline += self.single_gaussian(wavelength, self.linerest_n[i_linked], voff, fwhm, 
    ...
    def models_unitnorm_obsframe(self, wavelength, input_pars, if_pars_flat=True):
        ...
        for i_comp in range(self.num_comps):
            ...
            models_scomp = self.models_single_comp(wavelength, pars[i_comp], list_valid, list_free)
            if i_comp == 0: 
