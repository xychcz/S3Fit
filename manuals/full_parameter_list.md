...
- `model_R_ratio` (float, optional) \
   In order to save the memory usage and accelerate the running speed, the model templated will be smoothed and downsampled to match the sampling and resulotion of the input data.
   `model_R_ratio` is the ratio of modified model resolution and data instrumental resolution (set with `spec_R_inst_w`).
